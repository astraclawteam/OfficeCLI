...
public partial class WordHandler : IDocumentHandler, Rendering.IRenderModelHost
{
    ...
    {
        // Opening an editable package or refusing a mutation must not replace
        // the original ZIP merely because the SDK reserialized its in-memory DOM.
        if (!Modified || _packageStream == null || _backingStream == null) return;
