    ...
    public List<DocumentIssue> ViewAsIssues(string? issueType = null, int? limit = null)
    {
        ...
        foreach (var (sheetName, worksheetPart) in GetWorksheets())
        {
            foreach (var chartPart in worksheetPart.DrawingsPart?.ChartParts ?? Enumerable.Empty<ChartPart>())
            {
                ...
                {
                    if (!ShouldScan(finding.Subtype)) continue;
                    if (limit.HasValue && issues.Count >= limit.Value) break;
                    issues.Add(new DocumentIssue
                    {
        ...
        foreach (var (sheetName, worksheetPart) in GetWorksheets())
        {
            ...
            if (ShouldScan(Core.IssueSubtypes.ExcelPrintLayout, IssueType.Format)
                && (!limit.HasValue || issues.Count < limit.Value)
                && maxColumn >= 8 && (!fitToOnePageWide || !printArea))
            {
                issues.Add(new DocumentIssue
                {
        ...
        if (ShouldScan(Core.IssueSubtypes.TextOverflow, IssueType.Format)
            && (!limit.HasValue || issues.Count < limit.Value))
        {
